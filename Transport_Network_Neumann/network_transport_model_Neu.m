...
F_edge_0_ = 1e-8*6*30*24*(60)^2;%1e-08; %Recruitment term edge
beta_ = 1e-06;
...
mu_r_0_ = 1e-07*time_scale_; %1.5552 %release at x=0
mu_r_L_ = mu_r_0_; %release at x=L
mu_u_0_ = 1e-07*time_scale_; %uptake at x=0
...
der(:,h)=(N(:,h+1)-N(:,h))./(t(h+1)-t(h))
 ...
 N(seedregions_,h+1) = N(seedregions_,h+1) + ip.Results.alpha*(t(h+1)-t(h)); % linear growth in seed region
  ...
  tic
[netw_flux_0(:,:,h+1),netw_flux_L(:,:,h+1),Mass_edge(:,:,h+1),F_source_edge(:,:,h+1),n_ss0(:,:,h+1),res_max(:,h+1)]=NetworkFluxCalculator_Neu(N_adj_h1,N(:,h+1),n_ss0(:,:,h),matdir,'beta',ip.Results.beta,...
                                    'delta',ip.Results.delta,'F_edge_0',ip.Results.F_edge_0,...
                                    'lambda1_x0',Lambda1_x0_h1,...
                                    'lambda1_xL',Lambda1(:,h+1),...
                                    'gamma1_x0',Gamma1_x0_h1,...
